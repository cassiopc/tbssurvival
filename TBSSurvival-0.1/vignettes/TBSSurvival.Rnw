...
First of all, we just show some simple tests with the MLE. These
following examples are only to demonstrate the use of the estimation function.

<<simple_mle,eval=FALSE>>=
set.seed(1)
...
## now discriminated by number of affected lymph nodes > 4
...
Getting back to the analysis of before, we build a formula with time
and censoring indication (delta) and then perform the estimation:
...
It is important say that for Bayesian estimation we need to be
concerned about the choose of the prior and the convergence of MCMC
method. Because that, probably will be necessary some modifications in
the code to adapt to your data. In the cases that we tested the simple
change in \verb=scale= parameter of the \verb=tbs.survreg.be= was
enough to use the Bayesian procedure. We are using a prior that
reflect our knowledge about the parameter of the model. More details
about that see Polpo et al. 2012.
...
This ``manual'' content only the basics about the TBSSurvival, we
suggest to the reader take a look on the help pages of the TBSSurvival
package. More details about the TBSSurvival can be obtained in Polpo
et al. 2012.
...
The source code used to run all the experiments and analysis appearing
in the article {\it Transform Both Sides Model: A Parametric Approach},
Polpo et al. 2012, is available within this package in the file
\verb=tests/article.r=.
