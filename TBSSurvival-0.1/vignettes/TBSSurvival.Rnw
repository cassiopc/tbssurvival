...
problem. For more details we suggest the accompanying technical paper:
...
normal (used with \verb=dist\=''norm''=), t-student (\verb=dist\=''t''=), Cauchy
(\verb=dist\=''cauchy''=), double-exponential (\verb=dist\=''doubexp''=) and
logistic (\verb=dist\=''logistic''=) distributions.
...
returned. As one can see, the output contains already some score criteria and
...
s=tbs.survreg.mle(Surv(colon$time,colon$status==1) ~ colon$age60,
      dist="norm",method=c("BFGS","Nelder-Mead"),verbose=FALSE)
str(s)
@
...
aux.survival <- matrix(0,length(tbs.be$time),size)
for (j in 1:size) {
  aux.survival[,j] <-     c(1-ptbs(tbs.be$time,lambda=tbs.be$post[j,1],xi=tbs.be$post[j,2],
                                   beta=tbs.be$post[j,3:length(tbs.be$post[1,])],x=x,dist=dist))
}
